...
def load_file(item, id2triple, inv_entity_vocab: dict, inv_rel_vocab: dict) -> dict:
        ...
        #     for t in item['subgraph']:
        if len(subgraph) != 0:
            for t in subgraph: #! t는 triple id임 
                u, r, v = id2triple[t] # triple # = 3791303
                if u not in inv_entity_vocab:
                    inv_entity_vocab[u] = entity_cnt
                    entity_cnt += 1
                if v not in inv_entity_vocab:
...
def make_gnn_first_input(item, id2triple):
    ...
    kg_triplets = kg_result["triplets"] #! len=5436 (0,1,0), (2,1,0), (3,4,1), (5,6,2), ...
    
    train_target_edges = torch.tensor([[t[0], t[1]] for t in kg_triplets], dtype=torch.long).t() #! [2,5436]
    train_edges = train_target_edges 
    train_target_etypes = torch.tensor([t[2] for t in kg_triplets]) #! [5436]
...
def make_gnn_second_input(question, path, topic_entity, target_entity, text_encoder, kg_data, encode_path=False):
    ...
    not_filtered_target_entity = list(set(target_entity) & set(ent2id.keys())) #1998개 entitiy
    ...
    question_entities_masks.append(entities_to_mask(question_entities, num_nodes)) # list([1998]) 1998개에서 question_entities만 1 나머지 0
    supporting_entities_masks.append(entities_to_mask(supporting_entities, num_nodes))

    question_entities_masks = torch.stack(question_entities_masks) # tensor size [1,1998]
    supporting_entities_masks = torch.stack(supporting_entities_masks)
    if encode_path:
        question_embeddings = text_encoder(question + path).cpu()
    else:
        question_embeddings = text_encoder(question).cpu() #! [1,1024]
