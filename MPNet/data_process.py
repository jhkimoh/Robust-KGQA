...
def new_load_data(path: str, graph_path, triple2id_path: str, max_num_neg: int, max_num_pos: int, tokenizer, max_query_hop: int = 4):
    ...
    for i in tqdm(range(len(data))):
        ...
        for sample in golden_triples: # golden_triples(path 1) in golden_paths가 더 정확할듯 
            query_hop = len(sample)
            if query_hop > max_query_hop:
                continue
        
            for hop_idx in range(query_hop):# a triple in a path
                ...
                if len(cand_triples) == 0:
                    error +=1
                    continue
               #! 275-280 align이 안됨 for t in cand_triples에서 실제 t의 head가 id여도 스파클골든패스에 따라 처리하기 때문에(sample) 
               #! 오히려 id아닌 헤드에 대해서도 sample[0][0]로 대체하는 경우 그리고 실제 id인 헤드에 대해 대체하지 않고 넘어가는 경우가 꽤 있을것으로 봄
                if len(sample[hop_idx][0]) > 1 and sample[hop_idx][0][1] == '.':
