...
from relation_retriever_fix import *
from Attn.fusion import *
import pickle
import ast
...
if __name__ == '__main__':
    ...
    parser.add_argument("--use_union", action='store_true', default = False)
    parser.add_argument("--use_attention", action='store_true', default = False)
    parser.add_argument("--use_3shot", action='store_true', default = False)
    ...
    parser.add_argument("--fusion_path", type=str, default='./ckpt/fusion.pt')
    ...
    if args.sample:
        random.seed(42)
        dataset = random.sample(list(dataset), 300)
    ...
    if args.use_attention:
        fusion = ScoreFusion(q_dim=1024, hidden_dim=256).to(GNN_device)
        fusion_state = torch.load(args.fusion_path, map_location=GNN_device)
        fusion.load_state_dict(fusion_state["model"])
        fusion.eval() 
    ...
    gnn_model.load_state_dict(state["model"], strict=False)
    gnn_model.to(GNN_device)

    ## 4) initial setting
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_file = f'{current_time}_{args.dataset}.jsonl'
    total_hit, total_f1 = [], [] 
    ...
    for inds in tqdm(range(len(dataset))):
        ...
        for topic_ent in filtered_keys:
            ...
            while True:
                rel_ent_dict, temp_rel_ent_dict = dict(), dict()
                cnt += 1
                #! sub_Q define
                if args.dataset == 'webqsp':
                    sub_Q = original_q
                ...
                # Relation Pruning
                if args.use_union:
                    result_log = run_pruning_experiment(model, sub_Q, topic_ent, cand_rel)
                    retrieved_rel = result_log['final_selection']
                else:
                    # Relation Pruning
                    if len(cand_rel) > 3:
                    ...
                    if retrieved_rel != ["None"]:
                        ...
                        if len(gnn_temp_rel_ent_dict) != 0:
                            ...
                            for rel, cand_ent in gnn_temp_rel_ent_dict.items():
                                ...
                                if cand_ent and count == len(cand_ent):
                                    check_cnt += 1
                        ...
                        if len(mpnet_temp_rel_ent_dict) != 0:
                            ...
                            if len(mpnet_input_triples) != 0:
                                ...
                                mpent_topk_triples, mp_entity2prob = calculate_entity_probabilities(sorted_scores, sorted_triples)
                                ...
                                if args.use_attention:
                                    with torch.no_grad():
                                        q_emb = text_encoder(sub_Q).to(GNN_device)
                                        w = fusion(q_emb)
                                    print(f"[Fusion] Attention weights (MPNet, GNN) = {w[0].item():.4f}, {w[1].item():.4f}")
                                    fused_dict, _ = fuse_and_train(
                                        q_text=sub_Q,
                                        mp_scores=mp_entity2prob,
                                        gnn_scores=entity2prob,
                                        gold_entities=set(),     # train_mode=False라 실제로 안 쓰임
                                ...
                                normalized_kg_keys = {k.lower().replace(" ", "").replace("_", ""): k for k in mp_entity_to_triple.keys()}
                                valid_keys = []
                                for k in half_result:
                                    # Case 1: Exact Match
                                    if k in mp_entity_to_triple:
                                        valid_keys.append(k)
                                        continue
                                    # Case 2: Part Match
                                    k_norm = k.lower().replace(" ", "").replace("_", "")
                                    if k_norm in normalized_kg_keys:
                                        original_key = normalized_kg_keys[k_norm] # KG에 있는 진짜 키로 복원
                                        valid_keys.append(original_key)

                                half_result = valid_keys
                                if half_result != ["None"] and len(half_result)>0: #!#! half_result=['Theodore Lesieg'] 토픽엔티티일경우 고려
        ...
        if len(path_map) !=0:
            ...
            mpent_topk_triples, final_mp_entity2prob = calculate_entity_probabilities(sorted_scores, sorted_triples)
        ...
        #with repacking
        if args.use_3shot:
            input_text = FINAL_ANSWER_PROMPT_new.format(Q=total_original_q, T='\n'.join(triplets))
        else:
